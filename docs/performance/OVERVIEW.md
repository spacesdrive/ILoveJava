...
With the Java Fundamentals lessons (ten lessons, `/learn/java-fundamentals/*`), the home route's own bundle is close to unchanged (~194 KB JS / ~61 KB gzip, up from ~184/59 after the homepage redesign added a hero, feature grid, how-it-works, and curriculum preview section - the increase is a handful of Lucide icons and new markup, not a new dependency) - the lesson-rendering machinery (Shiki, CodeMirror, the lesson/exercise/quiz features) is entirely behind the lazy-loaded lesson routes, per the lazy-load rule above. Those lesson-route chunks are large (roughly 480-540 KB / 150-170 KB gzip, dominated by `@uiw/react-codemirror` and `@codemirror/lang-java` for the exercise editor) - acceptable for now since they only load when a learner opens a lesson, but worth a closer look (manual chunking, deferring CodeMirror until "Run" is first needed) if that route's Lighthouse score doesn't hold up under measurement.

The homepage's scroll-reveal (`FeatureGrid`, `HowItWorks` in `src/pages/home`) deliberately does not use Framer Motion, unlike `LessonSteps` (`src/features/lessons`) - Framer Motion is only otherwise imported behind the lazy-loaded lesson routes, and importing it from the home page (which is not lazy-loaded) would pull the whole library into the eager entry bundle instead of the deferred chunk it currently shares with those routes. `src/pages/home/use-reveal.ts` gets the same on-scroll fade-in effect from a small `IntersectionObserver` hook plus `motion-safe:` Tailwind variants instead.
